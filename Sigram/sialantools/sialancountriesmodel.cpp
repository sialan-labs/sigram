...
SialanCountriesModel::SialanCountriesModel(QObject *parent) :
    QAbstractListModel(parent)
{
    p = new SialanCountriesModelPrivate;
    init_buff();
}

QString SialanCountriesModel::id(const QModelIndex &index) const
{
    int row = index.row();
    return p->list.at(row);
}

int SialanCountriesModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    return p->list.count();
}

QVariant SialanCountriesModel::data(const QModelIndex &index, int role) const
{
    QVariant res;
...
QHash<qint32, QByteArray> SialanCountriesModel::roleNames() const
{
...
int SialanCountriesModel::count() const
{
    return p->list.count();
}

void SialanCountriesModel::init_buff()
{
    QFile file(":/sialantools/files/countries.csv");
...
SialanCountriesModel::~SialanCountriesModel()
{
    delete p;
}
